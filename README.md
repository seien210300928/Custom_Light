![NeoForge](https://img.shields.io/badge/NeoForge-21.X-blue)
...
5. 未知原因导致退出服务器后文件不一定能及时删除，建议在进入单人存档时，检查本地是否还有服务器配置文件。若有，则可手动删除或者多次重进单人存档以保证服务器配置文件正确删除；<br>For unknown reasons, the file may not be deleted promptly after disconnecting from the server. It is recommended that upon entering a single-player world, you check whether the server configuration file still exists locally. If it does, you can manually delete it or re-enter the single-player world multiple times to ensure the server configuration file is properly deleted;